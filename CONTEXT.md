...
- Open Mischief windows in one VS Code profile share registered Threads and Thread attention state. Every Workspace row shows the highest-priority Thread indicator plus active and attention counts, while the Threads pane remains scoped to the current Workspace. Each Workspace window’s extension host still owns its running Threads; closing that window stops its in-flight turns.
